...
import { ArrowLeftOnRectangleIcon, BellIcon, ChartPieIcon, PuzzlePieceIcon, DocumentTextIcon, UsersIcon, PhotoIcon } from '@heroicons/react/24/outline';
...
interface AdminPanelProps {
    ...
    setForumMessages: React.Dispatch<React.SetStateAction<ForumMessage[]>>;
...
type AdminTab = 'dashboard' | 'matches' | 'content' | 'users' | 'ads';
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'dashboard':
                ...
                />;
            case 'matches':
                return <MatchManagement matches={props.matches} setMatches={props.setMatches} />;
            case 'content':
                return <ContentManagement rules={props.rules} setRules={props.setRules} info={props.info} setInfo={props.setInfo} />;
            case 'users':
                return <UserManagement forumMessages={props.forumMessages} setForumMessages={props.setForumMessages} contactMessages={props.contactMessages} setContactMessages={props.setContactMessages} users={props.users} />;
            case 'ads':
                return <AdsManagement ads={props.ads} setAds={props.setAds} />;
            default:
                return null;
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white flex">
            ...
            <main className="flex-1 flex flex-col">
                <header className="bg-gray-800/50 p-4 flex justify-between items-center">
                    <h2 className="text-2xl font-bold capitalize">{activeTab.replace('ads', 'Publicités')}</h2>
