...
export const UserManagement: React.FC<UserManagementProps> = ({ forumMessages, setForumMessages, contactMessages, setContactMessages, users }) => {
    ...
    const deleteForumMessage = (id: string) => {
        setForumMessages(prev => prev.filter(msg => msg.id !== id));
    };

    const banUser = (userId: string) => {
        alert(`L'utilisateur avec l'ID ${userId} a été banni (simulation).`);
    };
    ...
    const handleSendReply = () => {
        ...
        const adminReply: ContactMessage = {
            id: `cm_${Date.now()}`,
            user: selectedUser, // The conversation is with this user
            message: replyMessage,
            timestamp: new Date().toISOString(),
            isFromAdmin: true,
        };

        setContactMessages(prev => [...prev, adminReply]);
        setReplyMessage('');
    };
